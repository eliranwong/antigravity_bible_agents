---
name: Obad
description: Search for given words or phrases in the book of Obadiah in one or multiple bibles, with wildcards (*, ?) and logical combinations (+, |) supported.
---
...
- Always run the python retriever script located at `.agents/skills/search/search_retriever.py` with the `--book Obad` option to perform the search.
- Execute the script using: `python3 .agents/skills/search/search_retriever.py --book Obad "<query>"` where `<query>` is the input prompt or arguments.
