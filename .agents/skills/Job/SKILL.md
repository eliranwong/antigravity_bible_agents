...
- Always run the python retriever script located at `.agents/skills/search/search_retriever.py` with the `--book Job` option to perform the search.
- Execute the script using: `python3 .agents/skills/search/search_retriever.py --book Job "<query>"` where `<query>` is the input prompt or arguments.
